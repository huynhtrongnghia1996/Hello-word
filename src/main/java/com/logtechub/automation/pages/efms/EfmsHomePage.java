...
public class EfmsHomePage extends BasePage {
    ...
    @Step("Login to eFMS")
    public EfmsHomePage login(String username, String password) {
        waitForVisible(USERNAME_SELECTORS, "eFMS username input").fill(username);
        waitForVisible(PASSWORD_SELECTORS, "eFMS password input").fill(password);
        waitForVisible(SUBMIT_SELECTORS, "eFMS login submit button").click();
        waitForDomContentLoaded();
        return this;
    }
